...
def upgrade() -> None:
    ...
    # The agent feature will add its own columns in a subsequent migration.
    op.create_table(
        "agent_configs",
    ...
    op.execute("CREATE TYPE session_status AS ENUM ('active', 'archived')")
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            PGEnum(name="session_status", create_type=False),
            nullable=False,
    ...
    op.create_index(
        "ix_sessions_user_agent_updated",
        "sessions",
    ...
    op.create_table(
        ...
        sa.Column(
            "role",
            PGEnum(name="message_role", create_type=False),
            nullable=False,
        ),
