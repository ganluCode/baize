...
def _make_agent(system_prompt: str = "Hello, {{ user_name }}!", tools: list[str] | None = None) -> MagicMock:
    agent = MagicMock()
    # 新 schema：prompts 字典；测试沿用 system_prompt 参数名以简化改动，把它当作 behavior 层
    agent.prompts = {"behavior": system_prompt}
...
def _make_memories(contents: list[str]) -> list[Memory]:
    return [
        Memory(id=str(i), content=c, user_id="u-1", created_at=_NOW, updated_at=_NOW)
...
def test_user_preferences_absent_when_build_returns_empty():
    ...
    agent = _make_agent()
    user = _make_user(preferences=None)
    with patch("baize.context.system_prompt.build_user_preferences_prompt", return_value=""):
        result = assemble_system_prompt(agent, user)
    assert "用户偏好" not in result
...
def test_tools_layer_contains_registered_tool_name_and_description():
    ...
    with patch("baize.context.system_prompt.ToolRegistry", fresh_reg):
        result = assemble_system_prompt(agent, user)
...
def test_token_limit_truncates_preferences_layer():
    ...
    with patch("baize.context.system_prompt._count_tokens", side_effect=fake_count):
...
def test_no_truncation_when_within_token_limit():
    ...
    with patch("baize.context.system_prompt._count_tokens", return_value=100):
        result = assemble_system_prompt(agent, user)
